...
//fillTaskDropdown(this.value);
$form->addInput(array('type'=>'combobox',
  'onchange'=>'selectAssignedUsers(this.value);fillActivityDir(this.value);',
  'name'=>'project',
  'style'=>'width: 250px;',
  'data'=>$project_list,
  'datakeys'=>array('id','name'),
...
if ($user->canManageTeam() || $user->isClient()) {
  ...
  if ($user->canManageTeam())
    $users = ttTeamHelper::getUsers(); // Active and inactive users for managers. // update only active
  else if ($user->isClient())
...
$group_by_options['a_name'] = $i18n->getKey('form.reports.group_by_activity');
if ($user->canManageTeam() || $user->isClient())
{
