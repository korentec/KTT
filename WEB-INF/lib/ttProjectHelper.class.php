...
class ttProjectHelper {
  ...
  //hagay
  	function findAllProjectsCount($user, $restrict=false) {
		$result = array();
		 $mdb2 = getConnection();
  		...
  		if ($user->isManager()) {
			$sel_str = "SELECT count(*) FROM tt_projects AS p WHERE p.team_id = ". stripslashes_deep( $user->getUserId()) ." AND p.status=1";
  		} else {
    		$sel_str = "SELECT count(*) FROM tt_projects AS p WHERE p.team_id = ". stripslashes_deep( $user->getManagerId()) ." AND p.status=1";
  		}

  		if (!($user->isManager() || $user->isCoManager()) || $restrict ) {  //|| $user->isCoManager()
  			if (count($user->getProjects())>0) {
  				$arr = array();
  				foreach ($user->getProjects() as $p) {
  					if (isset($p["ub_checked"])) $arr[] = stripslashes_deep(  @$p["p_id"]);  //pear_db_quote
  				}
				$sel_str .= " AND  p.id in (".join(",",$arr).")";
  			} else {
  				$sel_str .= " AND 1=1";
  			}
		}
	...
	//Hagay
	function findAllProjects($user, $options = null)
  {
    //$restrict=false, $alldata=false, $showHidden=false
		$result = array();
		 $mdb2 = getConnection();

  		if ($user->isManager()) {
  			if (@$options['alldata']) {
  				$sel_str = "SELECT p.id as p_id, p.name as p_name, p.p_code,p.status as p_status,p.team_id as p_manager_id";
  			} else {
  				$sel_str = "SELECT p.id as p_id, p.name as p_name, p.p_code, p.team_id as p_manager_id";
  			}
  			$sel_str .=" FROM tt_projects AS p	WHERE p.team_id = ". $mdb2->quote($user->getManagerId())   ;
  			if (!@$options['showHidden']) $sel_str .= " AND p.status=1";
  		} else {
    		$sel_str = "SELECT p.id as p_id, p.name as p_name, p.p_code, p.status as p_status, p.team_id as p_manager_id FROM tt_projects AS p
    				WHERE p.team_id = ".   $mdb2->quote($user->getManagerId()) ;
    		if (!@$options['showHidden']) $sel_str .= " AND p.status=1";
  		}
