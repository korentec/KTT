...
class ttUserHelper {
  ...
  static function deleteBind($user_id, $project_id) {
    ...
    return true;
     ...
     function findProjectsBinded($user_id, $manager_id, $onlyChecked=true) {
		// echo "userID".$user_id;
     $mdb2 = getConnection();
      if (!$mdb2) return false;

      $result = array();
      
	   
           $sel_str = "select tt_user_project_binds.id as ub_id, tt_projects.id as p_id, tt_projects.name as p_name,
 tt_user_project_binds.project_id as ub_id_p,
 tt_user_project_binds.user_id as ub_id_u,
 rate as ub_rate, tt_user_project_binds.`status` as
 ub_checked,team_id as p_manager_id from tt_projects 
 left join tt_user_project_binds on (tt_projects.id=tt_user_project_binds.project_id and tt_user_project_binds.user_id=".$user_id.")
         where team_id=".$manager_id." and tt_projects.status=1".($onlyChecked ? " and tt_user_project_binds.`status`=1" : "");
  ...
  function getUserDetailsString($user, $i18n) {
    $pers_manager_info = "";
