...
class ttReportHelper {
  ...
  static function getWhere($bean) {
    ...
    $dropdown_parts = '';
    $subactivty=$bean->getAttribute('activity') ;
    ...
    if ($bean->getAttribute('activity')) $dropdown_parts .= ' and  l.al_activity_id  = '.$subactivty;
  ...
  static function getItems($bean) {
    ...
    array_push($fields, " DATE_FORMAT(l.date, '%d/%m/%Y') as date ");
    if($user->canManageTeam() || $user->isClient())
    ...
    if ($bean->getAttribute('chactivity')  )
      $left_joins .= " left join activities t on (t.a_id = l.al_activity_id)";		
    if ($include_cf_1) {
      if ($cf_1_type == CustomFields::TYPE_TEXT)
    ...
    if ('no_grouping' == $group_by_option || 'date' == $group_by_option)
      ...
      $sort_part .= $group_by_option.',  l.date ';
    ...
    switch ($group_by_option) {
      ...
      case 'task':
        ...
        $group_join = 'left join tt_tasks t on (l.task_id = t.id) ';
  	    break;
        case 'a_name':
  	    $group_field = 't.a_name';
        $group_join = 'left join activities t on (t.a_id = l.al_activity_id) ';
  	    break;
      case 'cf_1':
